...
int main(int argc, char **argv) {
    ...
    // Your code goes here
    int char_numbers_to_add;
    if (strlen(argv[1]) >= 20){
      char_numbers_to_add = 19;
    } else {
      char_numbers_to_add = strlen(argv[1]);
    }
    strncat(greeting, argv[1], char_numbers_to_add);
    strncat(greeting, " ", 20 - char_numbers_to_add-1);
    if (20 - char_numbers_to_add -2 >= 0){
      strncat(greeting, name, 20 - char_numbers_to_add -2);
    }
